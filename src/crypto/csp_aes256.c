...
// This implementation assumes a 256-bit key and 128-bit block size.
...
static inline void xor_bytes(uint8_t * dst, uint8_t * src, uint32_t len) {
    unsigned int i;
    for (i = 0; i < len; i++)
        dst[i] ^= src[i];
}
...
int csp_aes256_encrypt(uint8_t * plain, const uint32_t len, uint32_t iv[4]) {
    ...
    uint32_t blocks = (len + AES256_BLOCKSIZE - 1)/ AES256_BLOCKSIZE;
    uint32_t remain;

    /* Initialize stream */
    stream[0] = csp_htobe32(iv[0]);
    stream[1] = csp_htobe32(iv[1]);
    stream[2] = csp_htobe32(iv[2]);
    stream[3] = csp_htobe32(iv[3]);

    aes256_context state;
    aes256_init(&state, csp_aes256_key);

    for (i = 0; i < blocks; i++) {
        /* Create stream */
        aes256_encrypt_ecb(&state, (uint8_t *)stream);

        /* Calculate remaining bytes */
        remain = len - i * AES256_BLOCKSIZE;

        /* XOR plain text with stream to generate cipher text */
        xor_bytes(&plain[len - remain], (uint8_t *)stream, remain < AES256_BLOCKSIZE ? remain : AES256_BLOCKSIZE);
...
/**
 * Decrypt AES256 encrypted byte array
 * @param cipher Pointer to cipher text
 * @param len Length of plain text
 * @param iv Initialization vector
 */
int csp_aes256_decrypt(uint8_t * cipher, const uint32_t len, uint32_t iv[4]) {
    /* Since we use counter mode, we can reuse the encryption function */
    return csp_aes256_encrypt(cipher, len, iv);
}
